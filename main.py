...
class Obstacle(pygame.sprite.Sprite):
    ...
    def __init__(self, type):
        ...
        if self.type == 'cactus':
            ...
            self.rect = self.image.get_rect(bottomleft=(1700,370))
        else:
            ...
            self.image = self.obstacle[self.index]
            self.rect = self.image.get_rect(bottomleft=(1700,choice([350, 310, 270]))) # different hights so that the player could either jump, duck or stay as it is
    def move(self):
...
while True:
    ...
    if run:
        ...
        game_speed = 8 # making it back to its original value
